...
BOOL timing_timeSetEvent(UINT delayInMillis)
{
    ...
    // Create the timer
    idEvent = timeSetEvent(delayInMillis, uResolution, TimerFunction, 0, TIME_ONESHOT);
...
BOOL rdtsc_diff_locky()
{
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        // Did it take at least 10 times more CPU cycles to perform CloseHandle than it took to perform
        // GetProcessHeap()?
...
BOOL timing_IcmpSendEcho(UINT delayInMillis)
{
    ...
    IcmpSendEcho(hIcmpFile, DestinationAddress, SendData, sizeof(SendData), NULL, ReplyBuffer, ReplySize,
                 delayInMillis);
    IcmpCloseHandle(hIcmpFile);
    free(ReplyBuffer);
...
BOOL timing_CreateTimerQueueTimer(UINT delayInMillis)
{
    ...
    if (CreateTimerQueueTimer(&hTimerQueueTimer, hTimerQueue, &CallbackCTQT, reinterpret_cast<PVOID>(0xDEADBEEFULL),
                              delayInMillis, 0, WT_EXECUTEDEFAULT) == FALSE)
    {
        bResult = TRUE;
    }
    else
