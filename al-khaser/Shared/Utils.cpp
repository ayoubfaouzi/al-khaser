...
PVOID64 GetPeb64()
{
    ...
    if (API::IsAvailable(API_IDENTIFIER::API_NtWow64QueryInformationProcess64))
    {
        ...
        auto NtWow64QueryInformationProcess64 = static_cast<pNtWow64QueryInformationProcess64>(
            API::GetAPI(API_IDENTIFIER::API_NtWow64QueryInformationProcess64));
        NTSTATUS status = NtWow64QueryInformationProcess64(GetCurrentProcess(), ProcessBasicInformation, &pbi64,
                                                           sizeof(pbi64), nullptr);
...
BOOL GetOSDisplayString(LPTSTR pszOS)
{
    ...
    {
        auto GetNativeSystemInfo =
            static_cast<pGetNativeSystemInfo>(API::GetAPI(API_IDENTIFIER::API_GetNativeSystemInfo));
        ...
        else if (osvi.dwMajorVersion == 6)
        {
            ...
            if (osvi.dwMinorVersion == 1)
            {
                if (osvi.wProductType == VER_NT_WORKSTATION)
                    ...
                    StringCchCat(pszOS, MAX_PATH, TEXT("Windows Server 2008 R2 "));
            }
            ...
            if (osvi.wProductType != VER_NT_WORKSTATION)
            {
                ...
                else if (si.wProcessorArchitecture == PROCESSOR_ARCHITECTURE_AMD64)
                {
                    if (osvi.wSuiteMask & VER_SUITE_DATACENTER)
                        ...
                        StringCchCat(pszOS, MAX_PATH, TEXT("Standard x64 Edition"));
                }
...
UINT enum_system_firmware_tables(DWORD FirmwareTableProviderSignature, PVOID pFirmwareTableBuffer, DWORD BufferSize)
{
    ...
    auto EnumSystemFirmwareTables =
        static_cast<pEnumSystemFirmwareTables>(API::GetAPI(API_IDENTIFIER::API_EnumSystemFirmwareTables));
...
PBYTE get_system_firmware(_In_ DWORD signature, _In_ DWORD table, _Out_ PDWORD pBufferSize)
{
    ...
    auto GetSystemFirmwareTable =
        static_cast<pGetSystemFirmwareTable>(API::GetAPI(API_IDENTIFIER::API_GetSystemFirmwareTable));
...
{
    auto NtWow64ReadVirtualMemory64 =
        static_cast<pNtWow64ReadVirtualMemory64>(API::GetAPI(API_IDENTIFIER::API_NtWow64ReadVirtualMemory64));
...
std::vector<PMEMORY_BASIC_INFORMATION64> *enumerate_memory_wow64()
{
    ...
    auto NtWow64QueryVirtualMemory64 =
        static_cast<pNtWow64QueryVirtualMemory64>(API::GetAPI(API_IDENTIFIER::API_NtWow64QueryVirtualMemory64));
    ...
    while (addr < MaxAddress)
    {
        ...
        NTSTATUS status;
        if ((status = NtWow64QueryVirtualMemory64(GetCurrentProcess(), (PVOID64) addr, 0, mbi,
...
std::vector<wchar_t *> *enumerate_object_directory(const wchar_t *path)
{
    ...
    while (ntQueryDirectoryObject(hDirectory, pObjDirInfo, 0x800, TRUE, FALSE, &context, &returnedLength) == 0 &&
           returnedLength > 0)
    {
