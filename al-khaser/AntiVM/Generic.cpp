...
VOID known_file_names()
{
    ...
    CONST TCHAR *szFilenames[] = {
        _T("sample.exe"), _T("bot.exe"),    _T("sandbox.exe"), _T("malware.exe"),
        _T("test.exe"),   _T("klavme.exe"), _T("myapp.exe"),   _T("testapp.exe"),
    ...
    for (int i = 0; i < dwlength; i++)
    {
        _stprintf_s(msg, sizeof(msg) / sizeof(TCHAR), _T("Checking if process file name contains: %s "),
                    szFilenames[i]);
    ...
    _stprintf_s(msg, sizeof(msg) / sizeof(TCHAR), _T("Checking if process file name looks like a hash: %s "),
                szFileName);
...
VOID other_known_sandbox_environment_checks()
{
    ...
    if ((0 == StrCmp(username, _T("Wilber"))) &&
        ((0 == StrCmpNI(NetBIOSHostName, _T("SC"), 2)) || (0 == StrCmpNI(NetBIOSHostName, _T("SW"), 2))))
    {
        matched = TRUE;
    }
    print_results(matched, (TCHAR *) _T("Checking whether username is 'Wilber' and ")
    ...
    matched = FALSE;
    if ((0 == StrCmp(username, _T("John"))) && (is_FileExists((TCHAR *) _T("C:\\take_screenshot.ps1"))) &&
    ...
    matched = FALSE;
    if ((is_FileExists((TCHAR *) _T("C:\\email.doc"))) && (is_FileExists((TCHAR *) _T("C:\\email.htm"))) &&
        (is_FileExists((TCHAR *) _T("C:\\123\\email.doc"))) && (is_FileExists((TCHAR *) _T("C:\\123\\email.docx"))))
    {
        matched = TRUE;
    }
    ...
    matched = FALSE;
    if ((is_FileExists((TCHAR *) _T("C:\\a\\foobar.bmp"))) && (is_FileExists((TCHAR *) _T("C:\\a\\foobar.doc"))) &&
                ...
                if (SUCCEEDED(hRes))
                {
                    if (V_VT(&vtProp) != VT_NULL)
                    {
                        ...
                        unsigned long long diskSizeBytes =
                            _tcstoui64_l(vtProp.bstrVal, NULL, 10, _get_current_locale());
                        // do the check only if we successfuly got the disk size
                        if (errno == 0)
                        {
                            // Do our comparison
                            if (diskSizeBytes < minHardDiskSize)
                            { // Less than 80GB
                                bFound = TRUE;
                            }
                        }
...
BOOL dizk_size_deviceiocontrol()
{
    ...
    if (winDirLen)
    {
        ...
        if (driveNumber >= 0)
        {
            ...
            HANDLE hVolume = CreateFile(driveRootPathBuffer, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
            ...
            if (hVolume != INVALID_HANDLE_VALUE)
            {
                ...
                if (diskExtents)
                {
                    ...
                    DWORD dummy = 0;
                    BOOL extentsIoctlOK = DeviceIoControl(hVolume, IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS, NULL, 0,
                    ...
                    if (extentsIoctlOK && diskExtents->NumberOfDiskExtents > 0)
                    {
                        ...
                        {
                            if (wnsprintf(physicalPathBuffer, MAX_PATH, _T("\\\\.\\PhysicalDrive%u"),
                                          diskExtents->Extents[i].DiskNumber) > 0)
                            {
                                // open the physical disk
                                hDevice = CreateFile(physicalPathBuffer, GENERIC_READ, FILE_SHARE_READ, NULL,
...
BOOL setupdi_diskdrive()
{
    ...
    for (i = 0; SetupDiEnumDeviceInfo(hDevInfo, i, &DeviceInfoData); i++)
    {
        ...
        {
            // Do our comparison
            if ((StrStrI(buffer, _T("vbox")) != NULL) || (StrStrI(buffer, _T("vmware")) != NULL) ||
                (StrStrI(buffer, _T("qemu")) != NULL) || (StrStrI(buffer, _T("virtual")) != NULL))
            {
                bFound = TRUE;
                break;
            }
            ...
            while (pEnumerator)
            {
                ...
                if (SUCCEEDED(hRes))
                {
                    if (vtProp.vt == VT_BSTR)
                    {
                        ...
                        // Do our comparison
                        if ((StrStrI(vtProp.bstrVal, _T("VMWare")) != 0) || (StrStrI(vtProp.bstrVal, _T("Xen")) != 0) ||
...
BOOL query_license_value()
{
    auto RtlInitUnicodeString =
        static_cast<pRtlInitUnicodeString>(API::GetAPI(API_IDENTIFIER::API_RtlInitUnicodeString));
    ...
    NTSTATUS Status =
        NtQueryLicenseValue(&LicenseValue, NULL, reinterpret_cast<PVOID>(&Result), sizeof(ULONG), &ReturnLength);
...
{
    int count = wmi_query_count(_T("SELECT * FROM ")
...
BOOL registry_disk_enum()
{
    ...
    for (unsigned int i = 0; i < dwEntriesLength; i++)
    {
        ...
        if (RegQueryInfoKey(hkResult, NULL, NULL, NULL, &cSubKeys, &cbMaxSubKeyLen, NULL, NULL, NULL, NULL, NULL,
                            NULL) != ERROR_SUCCESS)
        {
            RegCloseKey(hkResult);
            continue;
...
BOOL handle_one_table(BYTE *currentPosition, UINT &bias, BYTE *smBiosTableBoundary)
{
    ...
    while (!(currentPosition[i] == 0 && currentPosition[i + 1] == 0) && (currentPosition + i + 1 < smBiosTableBoundary))
    {
        i++;
    }
