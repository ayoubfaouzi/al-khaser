...
/*
 * This check uses the MEM_WRITE_WATCH feature of VirtualAlloc to test for additional memory writes by debuggers,
 * sandboxing, etc. There are a few ways that we can exploit this: (1) Allocate a buffer, write it once, get the count,
 * see if it's >1 (2) Allocate a buffer, pass it to an API where we know the buffer isn't touched (e.g. call with
 * invalid parameter) and see if the count is >0 (3) Allocate a buffer, use it to store a result of a call we care about
 * (e.g. IsDebuggerPresent) and see if the memory was hit exactly once (4) Allocate an executable buffer, copy a debug
 * check routine to it, run the check, see if any writes were performed after ours.
 *
...
BOOL VirtualAlloc_WriteWatch_BufferOnly()
{
    ...
    PVOID *addresses =
        static_cast<PVOID *>(VirtualAlloc(NULL, 4096 * sizeof(PVOID), MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE));
    if (addresses == NULL)
    {
    ...
    int *buffer =
        static_cast<int *>(VirtualAlloc(NULL, 4096 * 4096, MEM_RESERVE | MEM_COMMIT | MEM_WRITE_WATCH, PAGE_READWRITE));
    if (buffer == NULL)
    {
...
BOOL VirtualAlloc_WriteWatch_APICalls()
{
    ...
    if (GetEnvironmentVariable(L"%ThisIsAnInvalidEnvironmentVariableName?[]<>@\\;*!-{}#:/~%", (LPWSTR) buffer,
                               4096 * 4096) != FALSE)
    {
    ...
    if (error == FALSE)
    {
        ...
        {
            // should have zero reads here because GlobalGetAtomName doesn't probe the buffer until other checks have
            // succeeded if there's an API hook or debugger in here it'll probably try to probe the buffer, which will
            // be caught here
            result = hitCount != 0;
        }
    }
...
BOOL VirtualAlloc_WriteWatch_CodeWrite()
{
    ...
    byte *buffer = static_cast<byte *>(
        VirtualAlloc(NULL, 4096 * 4096, MEM_RESERVE | MEM_COMMIT | MEM_WRITE_WATCH, PAGE_EXECUTE_READWRITE));
    if (buffer == NULL)
    {
        VirtualFree(addresses, 0, MEM_RELEASE);
        printf("VirtualAlloc failed. Last error: %u\n", GetLastError());
