...
DWORD GetParentProcessId()
{
    ...
    Status = NtQueryInfoProcess(GetCurrentProcess(), ProcessBasicInformation, (PVOID) &pbi,
...
BOOL IsParentExplorerExe()
{
    ...
    if (parentPid > 0)
    {
        ...
        if (hParent != INVALID_HANDLE_VALUE)
        {
            ...
            if (GetModuleFileNameExW(hParent, NULL, parentProcessPath, MAX_PATH) &&
        ...
        // this check is less ideal because it throws false positives if you have explorer process isolation enabled
        // (i.e. one process per explorer window)
