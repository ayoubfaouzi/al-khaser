...
// Normalizes the path returned by GetProcessImageFileName
{
    ...
    *pszSlash = 0;
    ...
    for (wchar_t cDrive = 'A'; cDrive < 'Z'; ++cDrive)
    {
        szDrive[0] = cDrive;
        szNTPath[0] = 0;
        if (0 != QueryDosDevice(szDrive, szNTPath, NUMCHARS(szNTPath)) && 0 == _wcsicmp(szNTPath, pszPath))
        {
            // Match
...
// Normalizes the path returned by GetProcessImageFileName
{
    ...
    *pszSlash = 0;
    ...
    for (TCHAR cDrive = 'A'; cDrive < 'Z'; ++cDrive)
    {
        szDrive[0] = cDrive;
        szNTPath[0] = 0;
        if (0 != QueryDosDevice(szDrive, szNTPath, NUMCHARS(szNTPath)) && 0 == StrCmpI(szNTPath, pszPath))
        {
            // Match
...
bool IsBadLibrary(TCHAR *filename, DWORD filenameLength)
{
    ...
    if (GetEnvironmentVariable(_T("SystemDrive"), systemDrive, MAX_PATH) > 0)
    {
        if (QueryDosDeviceW(systemDrive, systemDriveDevice, MAX_PATH) > 0)
        {
            ...
            if (StrNCmpI(systemDriveDevice, filename,
                         (int) (min(systemDriveDevicelength, filenameLength) / sizeof(TCHAR))) == 0)
            {
            ...
            if (StrNCmpI(systemRootPath, normalisedPath,
                         (int) (min(systemRootPathLength, normalisedPathLength) / sizeof(TCHAR))) == 0)
            {
...
#ifdef _X86_
            if (IsWoW64() && StrNCmpI(syswow64Path, normalisedPath,
                                      (int) (min(syswow64PathLength, normalisedPathLength) / sizeof(TCHAR))) == 0)
            {
...
BOOL ScanForModules_EnumProcessModulesEx_Internal(DWORD moduleFlag)
{
    ...
    {
        fnEnumProcessModulesEx =
            static_cast<pEnumProcessModulesEx>(API::GetAPI(API_IDENTIFIER::API_EnumProcessModulesEx_PSAPI));
    }
    else
    {
        fnEnumProcessModulesEx =
            static_cast<pEnumProcessModulesEx>(API::GetAPI(API_IDENTIFIER::API_EnumProcessModulesEx_Kernel));
    }
    ...
    if (moduleList)
    {
        ...
        if (fnEnumProcessModulesEx(GetCurrentProcess(), moduleList, currentSize, &requiredSize, moduleFlag))
        {
            ...
            if (requiredSize > currentSize)
            {
                ...
                {
                    moduleList = tmp;
                    if (fnEnumProcessModulesEx(GetCurrentProcess(), moduleList, currentSize, &requiredSize,
                                               moduleFlag) == FALSE)
                    {
                        success = false;
                    }
    ...
    for (PMEMORY_BASIC_INFORMATION region : *memoryRegions)
    {
        ...
        while (addr < regionEnd)
        {
            ...
            if (GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                                  (TCHAR *) addr, &moduleHandle) == FALSE)
            {
                // not a known module
                if ((region->State & MEM_COMMIT) == MEM_COMMIT &&
                    ((region->Protect == PAGE_READONLY) || (region->Protect == PAGE_READWRITE) ||
                     (region->Protect == PAGE_EXECUTE_READ) || (region->Protect == PAGE_EXECUTE_READWRITE) ||
...
BOOL ScanForModules_DotNetModuleStructures()
{
    ...
    /*
    This works because the .NET runtime loads structures into memory that describe modules. This happens even if the
    module is loaded dynamically, from memory. If al-khaser were a .NET application we'd need to apply some additional
    checks on the results, but since it isn't then we can just report every .NET module we find. This check is quite
    effective because it catches pretty much any kind of .NET injection, even if the injector uses tricks like messing
    with PE headers or patching EWT.
    */
    ...
    for (PMEMORY_BASIC_INFORMATION region : *memoryRegions)
    {
        ...
        if ((region->State & MEM_COMMIT) == MEM_COMMIT && region->Protect == PAGE_READWRITE &&
            ...
            if (*addr == 0)
            {
                ...
                while (addr < regionEnd - 32)
                {
                    ...
                    if (sixZeroes)
                    {
                        ...
                        ptr++;
                        if (*ptr++ == 1 && *ptr++ == 2 && *ptr++ == 0 && *ptr++ == 2 && *ptr++ == 0 && *ptr++ == 2 &&
                            *ptr++ == 0)
                        {
...
BOOL ScanForModules_LDR_Direct()
{
    ...
    auto NtQueryInformationProcess =
        static_cast<pNtQueryInformationProcess>(API::GetAPI(API_IDENTIFIER::API_NtQueryInformationProcess));
    NTSTATUS status =
            ...
            if (IsWoW64())
            {
                ...
                if (peb64 && attempt_to_read_memory_wow64(&ldrData, sizeof(PEB_LDR_DATA64), peb64->Ldr))
                {
                    ...
                    for (LDR_DATA_TABLE_ENTRY64 *ldrEntry : *ldrEntries)
                    {
                        ...
                        if (attempt_to_read_memory_wow64(dllNameBuffer, ldrEntry->FullDllName.Length * sizeof(WCHAR),
                                                         ldrEntry->FullDllName.Buffer))
                        {
                            // printf(" -> %S\n", dllNameBuffer);
...
BOOL ScanForModules_LdrEnumerateLoadedModules()
{
    ...
    auto LdrEnumerateLoadedModules =
        static_cast<pLdrEnumerateLoadedModules>(API::GetAPI(API_IDENTIFIER::API_LdrEnumerateLoadedModules));
