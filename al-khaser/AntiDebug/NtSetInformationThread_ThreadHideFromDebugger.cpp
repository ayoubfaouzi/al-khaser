...
{
    // this is needed because the bool data type can be at unaligned memory locations, whereas the
    // NtQueryInformationThread API expects 32-bit aligned pointers.
    struct AlignedBool
    {
    ...
    auto NtSetInformationThread =
        static_cast<pNtSetInformationThread>(API::GetAPI(API_IDENTIFIER::API_NtSetInformationThread));
    auto NtQueryInformationThread =
        static_cast<pNtQueryInformationThread>(API::GetAPI(API_IDENTIFIER::API_NtQueryInformationThread));
    ...
    // First issue a bogus call with an incorrect length parameter. If it succeeds, we know NtSetInformationThread was
    // hooked.
    Status = NtSetInformationThread(GetCurrentThread(), ThreadHideFromDebugger, &isThreadHidden, 12345);
    if (Status == 0)
        return TRUE;

    // Next try again but give it a bogus thread handle. If it succeeds, again we know NtSetInformationThread was
    // hooked.
    ...
    if (Status == 0)
    {
        if (doQITcheck)
        {
            ...
            // if a BOOL is used, the kernel returns 0xC0000004 (STATUS_INFO_LENGTH_MISMATCH) because BOOL is typedef
            // int.

            // first do a legitimate call. this should succeed or return an error such as access denied.
            Status = NtQueryInformationThread(GetCurrentThread(), ThreadHideFromDebugger, &isThreadHidden.Value,
                                              sizeof(bool), NULL);

            // this shouldn't happen, because we used the correct length. this will only happen if a buggy hook
            // mistakenly expects a BOOL rather than a bool.
            ...
            if (Status == 0)
            {
                ...
                // now do a bogus call with the wrong size. this will catch buggy hooks that accept BOOL (4 bytes) or
                // just don't have any size checks
                Status = NtQueryInformationThread(GetCurrentThread(), ThreadHideFromDebugger,
                                                  &bogusIsThreadHidden.Value, sizeof(BOOL), NULL);
                if (Status != STATUS_INFO_LENGTH_MISMATCH)
                {
                ...
                // NtQueryInformationThread explicitly requires the ThreadInformation pointer to be aligned. as such, it
                // should reject unaligned pointers. hooks are almost certainly guaranteed to not retain this behaviour,
                // so it's a very nice way to catch them out.
...
#else
                // on 32-bit, there should be either two or four aligned elements (unsure how WoW64 affects this, so I'm
                // just gonna assume 2 or 4 are ok)
                const size_t MaxAlignmentCheckSuccessCount = 4;
#endif
                for (size_t i = 0; i < UnalignedCheckCount; i++)
                {
                    Status = NtQueryInformationThread(GetCurrentThread(), ThreadHideFromDebugger,
                                                      &(bogusUnalignedValues[i]), sizeof(BOOL), NULL);
                    if (Status == STATUS_DATATYPE_MISALIGNMENT)
                    {
                        alignmentErrorCount++;
                    }
                }
                // if there weren't enough alignment errors, we know that the API must be hooked and not checking
                // alignment properly!
                ...
                // the legitimate call was successful, and the bogus call was unsuccessful, so return false (no
                // detection) if the HideFromDebugger flag was properly set. if the HideFromDebugger flag was not set,
                // i.e. the NtSetInformationThread call lied to us about being successful, then return true
